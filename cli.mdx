...
```bash
deepvista notes +quick "Your insight here"
```
...
Installing the CLI writes one consolidated **`deepvista`** skill into your agent's skills
directory — `~/.claude/skills/` by default, and also `~/.cursor/skills/`,
`~/.opencode/skills/`, or `~/.agents/skills/` when those directories exist. Your agent
loads the index first, then pulls only the reference file it needs for the job at hand.

| Reference | What it covers |
| --- | --- |
| `shared.md` | Auth, profiles, global flags, exit codes |
| `project.md` | Listing and switching the working project |
| `notes.md` | Quick capture, create, list, update, delete notes |
| `vistabase-card.md` | Search, create, pin, archive, upload cards; version history |
| `chat.md` | Chatting with the agent and managing sessions |
| `skill.md` | Listing, running, creating, and catalog-syncing Skills |
| `session.md` | Recording an agent conversation as a rolling session card |
| `tasks.md` | Running work dispatched to this machine |
| `pull.md` / `push.md` | Installing a Skill's bundled files, and uploading your own |

Your own project Skills install on top of that. `deepvista skill sync` writes each one
into the same directory as a thin `dv-<slug>` stub, so your agent can invoke a Skill you
built in the web app by name:
...
<AccordionGroup>
  ...
  <Accordion title="Research and synthesize with a Skill" icon="flask">
...
<CardGroup cols={2}>
  <Card title="Research synthesis" icon="magnifying-glass">
    ```text
    Search my memory for [topic] and run my Research Synthesis Skill
    ```
  </Card>
  ...
  <Card title="Interview debrief" icon="users">
    ```text
    I just finished a user interview. Run my Interview Debrief Skill with these notes: [notes]
    ```
  </Card>
...
<Tabs>
  ...
  <Tab title="Notes">
    ...
    | `deepvista notes create` | Create a note with a title and body |
    | `deepvista notes list` | List recent notes |
    | `deepvista card +search "<query>" --type note` | Search notes by keyword or semantic query |
  </Tab>
  <Tab title="Cards">
    | Command | Description |
    | --- | --- |
    | `deepvista card +search "<query>"` | Search context cards semantically |
    | `deepvista card get <card-id>` | Retrieve a specific card by ID |
    | `deepvista card create` | Create a new context card |
    | `deepvista card update <card-id>` | Update an existing card |
  </Tab>
  <Tab title="Skills">
    | Command | Description |
    | --- | --- |
    | `deepvista skill list` | List available Skills in your project |
    | `deepvista skill run <skill-id>` | Run a Skill by ID |
    | `deepvista skill get <skill-id>` | Inspect a Skill and its run status |
  </Tab>
</Tabs>
