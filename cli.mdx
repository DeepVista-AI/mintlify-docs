...
<Note>
  If Python 3.12 isn't available on your machine, install it first before running the command above. On macOS, start with [Mac setup](/tutorials/mac-setup).
</Note>

<Warning>
  The install script runs remotely fetched shell code. Prefer reviewing the script URL in a browser first if your environment requires it.
</Warning>
...
Use `deepvista notes +` for structured note entry, or `deepvista notes +quick "..."` for a one-line capture:
