...
Every action is scoped to one project. Unless your client says otherwise, that project is your **default project** — the one under **Settings → User → Default project** — not whichever project you happen to have open in the app. See [Choosing a project](#choosing-a-project).
...
Your cards live in projects, and an MCP connection reads and writes in exactly one of them.

- **No header sent** — the server uses your **default project**. Change it under **Settings → User → Default project**. Switching the project you are *viewing* in the app does not move the MCP connection.
- **`X-Project-Id` header sent** — the server uses that project, after checking you have access to it. The value can be the project's ID or its slug.

Clients that let you set request headers can target a project directly. For Claude Code:
...
For any client that cannot send headers, set the project you want as your default instead.
...
| **Delete** (permanent) | `Delete the context card titled Old vendor contract` |

<Warning>
  **Deleting over MCP is permanent.** It removes the card, its comments, and its links to other cards from the database. This is not the **Trash** you see in the app — there is nothing to restore afterwards, and unlike an edit, a delete has no version history to roll back to.

  Whether you get a confirmation prompt is up to your AI client, not DeepVista. Keep tool approval enabled for `delete_context_card` rather than allowing it automatically.
</Warning>

<Note>
  To point the connection at a different project, see [Choosing a project](#choosing-a-project).
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="The AI connects but can't see any of my cards" icon="eye-slash">
    The server reads from one project, and by default that is your **default project** — not the project you currently have open in the app. Check **Settings → User → Default project**, or send an `X-Project-Id` header from your client to target a different one. See [Choosing a project](#choosing-a-project). An empty list is expected if that project has no cards yet.
  </Accordion>
