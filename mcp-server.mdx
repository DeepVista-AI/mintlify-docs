...
<Tabs>
  <Tab title="Cursor">
    ...
    <Steps>
      <Step title="Save the config and restart Cursor">
        Save `~/.cursor/mcp.json`, then fully quit and reopen Cursor so it reloads MCP servers.
      </Step>
  ...
  <Tab title="VS Code">
    ...
    Name the server **deepvista** and press **Enter**. Then activate it via **MCP: List Servers** → select **deepvista** → **Start Server**.
    ...
    <Check>
      **Verify it's working:** In VS Code chat, ask `List my DeepVista context cards.` You should see cards from your active project.
    </Check>
  </Tab>
  <Tab title="Windsurf">
    ...
    Save the settings, restart Windsurf if prompted, and complete the OAuth sign-in when your browser opens.

    <Check>
      **Verify it's working:** In Cascade, ask `Show me my recent DeepVista context cards.` A list from your active project confirms the connection.
    </Check>
  </Tab>
...
Example Cursor config using a key instead of interactive OAuth:
...
<Warning>
  Treat API keys like passwords. Prefer OAuth for interactive editors, rotate keys if they're shared or committed, and never paste keys into chat logs.
</Warning>
